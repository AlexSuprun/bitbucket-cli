...
export class ViewPRCommand extends BaseCommand<{ id: string } & ViewPROptions, void> {
  ...
  private renderBranchInfo(pr: Pullrequest): void {
    const source = pr.source as { branch?: { name?: string }; commit?: { hash?: string } } | undefined;
    const destination = pr.destination as { branch?: { name?: string }; commit?: { hash?: string } } | undefined;
    ...
    if (source?.commit?.hash || destination?.commit?.hash) {
      const sourceHash = source?.commit?.hash ? chalk.gray(source.commit.hash.slice(0, 7)) : chalk.gray("unknown");
      const destHash = destination?.commit?.hash ? chalk.gray(destination.commit.hash.slice(0, 7)) : chalk.gray("unknown");
      this.output.text(`${chalk.dim("Commits:")}  ${sourceHash}${arrow}${destHash}`);
    }
  }

  private renderMetadata(pr: Pullrequest): void {
    this.output.text(`${chalk.dim("Author:")}   ${pr.author?.display_name ?? "Unknown"}`);
    ...
    if (mergeCommit?.hash) {
      this.output.text(`${chalk.dim("Merge:")}    ${chalk.magenta(mergeCommit.hash.slice(0, 7))}`);
    }
    ...
    this.output.text(`${chalk.dim("Activity:")} ${pr.comment_count ?? 0} comments · ${pr.task_count ?? 0} tasks`);
  }
  ...
  private renderReviewers(pr: Pullrequest): void {
    ...
    for (const reviewer of reviewers) {
      ...
      this.output.text(`  ${status.icon} ${reviewer.user?.display_name ?? "Unknown"} ${chalk.gray(status.label)}`);
    }
  }

  private getReviewerStatus(reviewer: {
    approved?: boolean;
    state?: string;
  ...
  private renderFooter(pr: Pullrequest): void {
    ...
    this.output.text("");
    this.output.text(chalk.gray("─".repeat(60)));
    this.output.text(`${chalk.dim("URL:")} ${chalk.blue.underline(links?.html?.href ?? "")}`);
    this.output.text("");
  }
